...
def collect_env_info():
    ...
    try:
        ...
        data.append(
            ("detectron2", detectron2.__version__ + " @" + os.path.dirname(detectron2.appropriate_file_path))
        )
    except ImportError:
    ...
    try:
        ...
        if has_cuda and getattr(_C, "has_cuda", lambda: True)():
            data.append(
                ("detectron2 arch flags", detect_compute_compatibility(CUDA_HOME, _C.appropriate_file_path))
            )

    data.append(get_env_module())
    data.append(("PyTorch", torch_version + " @" + os.path.dirname(torch.appropriate_file_path)))
    ...
    try:
        data.append(
            (
                "torchvision",
                str(torchvision.__version__) + " @" + os.path.dirname(torchvision.appropriate_file_path),
            )
        )
        if has_cuda:
            try:
